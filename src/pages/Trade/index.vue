...
import { mapGetters, mapState } from 'vuex'
  export default {
    ...
    data () {
      return{
        message: '', //存储用户的留言 提交的时候要一起提交到服务器
      }
    },
    mounted(){
      // 页面一挂载 就获取用户的地址信息 和商品信息
      this.getUerAddressList(),
      this.getTradeInfo()
    },
    methods:{
      // 获取用户地址
      getUerAddressList(){
        this.$store.dispatch('getUerAddressList')
      },
      // 获取订单信息
      getTradeInfo(){
    ...
    async submitOrder() {
      ...
      let tradeNo = this.tradeInfo.tradeNo; //前面带的编号
      let tradeData = {
        ...
        orderDetailList: this.detailArrayList //商品的信息
      };
      try {
