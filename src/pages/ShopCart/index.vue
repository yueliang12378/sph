  ...
  export default {
    ...
    methods:{
      ...
      async changeCartNum(shopCart,disNum,flag){
        //disNum 正数代表增加 负数代表减少 ，但是要确保 修改后数量还是要大于等于1

        if(!flag){ //flag为false 表示 这个是直接在input框输入的，得算出变化的量
          //针对输入的数据是最终的商品数量，我们得转化为变化的量
          //disNum表示变化的数量
          if(disNum > 0){  //disNum 表示最终结果是合法的
            disNum = disNum - shopCart.skuNum
          }else{  //最终的结果小于等于0 则
            disNum = 1 - shopCart.skuNum //为负数，表示减少1-shopCart.skuNum个商品
          }
        }else{
      ...
      async updateOne(shopCart){
        try {
          // 0表示false 1 表示true 如果shopCart.isChecked是ture 这发送0 反之发送1
      ...
      // 这里才是真正拿到商品数据的函数（同样也是为了防止假报错）
      shopCartList(){
        return this.shopCarInfo.cartInfoList || []
      },
      // 选中的商品的数量
      checkedNum(){
        // 利用reduce prev是当前计算的总数 回调函数后一个0是prev的初始值  item是每一项
        return this.shopCartList.reduce((prev,item) => {
          if(item.isChecked){ //如果是选中的 则添加上数量
            prev += item.skuNum
          }
          return prev
        },0)
      ...
      allMoney(){
        return this.shopCartList.reduce((prev,item)=>{
          if(item.isChecked){ //如果是选中的 则添加上数量
            prev += item.skuNum * item.cartPrice //加上价格
          }
          return prev
        },0)
      },
      isAllCheck:{
        ...
        async set(val){
          ...
          try {
             await this.$store.dispatch('updatedCartIscheckAll',val?1:0)
            // alert('修改所有的状态成功')
            this.getCartList()
          } catch (error) {
            alert(error.message)
