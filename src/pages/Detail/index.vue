...
<!-- 商品属性 -->
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
<!-- 输入框的失去焦点时间 blur ， 如果失去焦点时候内容没有改变 同样会执行一次回调 这样并不好
  我们可以利用change事件，如果失去焦点的时候，内容没有改变，则不会执行回调，如果内容改变了 则会执行，相比之下效率较高
-->            <input autocomplete="off" 
                v-model="skuNum" 
                class="itxt"
                @change='$event.target.value > 1?skuNum = $event.target.value : skuNum = 1'
               >
               <!-- 这里 $event.target.value > 1? 如果输入字符串和负数 他会自动转换 变成小于1的 那么skuNum = 1 -->
    ...
    methods:{
      ...
      async addShopCart(){
        ...
        try {
          ...
          //因为如果await 接收到的是一个失败的promise 会报错
          //所以如果请求失败 会执行catch函数
          alert('添加购物车成功')
        ...
        // 区别： localStorage是永久存储    sessionStorage浏览器关闭就没了
          //这里只是用于当前展示 所以用临时存储（后面会直接从服务器拿到数据的）
